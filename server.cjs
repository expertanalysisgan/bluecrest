...
const PAGE_ALIASES = {
    ...
    "/deposit": "dist/index.html",
    "/wire-transfer": "dist/index.html",
    "/dom-transfer": "dist/index.html",
    "/loan": "dist/index.html",
    "/transaction": "dist/index.html",
    "/profile": "dist/index.html",
    "/transfer": "dist/index.html",
    "/cards": "dist/index.html",
    "/notifications": "dist/index.html",
    "/summary": "dist/index.html",
    "/stocks": "dist/index.html",
};
    ...
    try {
        if (req.method === "GET" && url.pathname === "/") {
            ...
            if (fs.existsSync(publicIndex)) {
                return serveFile(res, publicIndex);
            }
        ...
        if (req.method === "GET") {
            ...
            const distPath = safeJoin(path.join(ROOT, "dist"), decodeURIComponent(url.pathname));
            if (distPath && fs.existsSync(distPath) && fs.statSync(distPath).isFile()) {
                return serveFile(res, distPath);
            }
            const publicPath = safeJoin(path.join(ROOT, "public"), decodeURIComponent(url.pathname));
            if (publicPath && fs.existsSync(publicPath) && fs.statSync(publicPath).isFile()) {
                return serveFile(res, publicPath);
            }
            const rootPath = safeJoin(ROOT, decodeURIComponent(url.pathname));
...
async function handleLogin(res, form) {
    const loginId = (form.acct_number || "").trim();
    const password = form.password || "";
    console.log("LOGIN REQUEST => loginId:", loginId, "password:", password);
    const user = await dbGet("SELECT * FROM users WHERE account_number = $1 OR email = $2", [loginId, loginId]);

    if (!user || !verifyPassword(password, user.password_hash)) {
        return redirect(res, "/auth/login?error=Invalid+account+number+or+password");
    }
...
async function handleLogout(req, res) {
    ...
    res.writeHead(302, {
        Location: "/auth/login?success=Signed+out+successfully",
...
async function requireSession(req, res) {
    const session = await getSessionUser(req);
    if (!session) {
        redirect(res, "/auth/login?error=Please+sign+in+first");
        return null;
    }
    return session;
}
